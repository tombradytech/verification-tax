...
import { CHART_CSS, cardSpecFor, chartCard, pointsFor } from './charts.js';
...
export function renderHtml(
  ...
  /** When set, the body becomes one person's charts instead of the ledger. */
  person?: { login: string; backHref: string; cards: string }
): string {
  ...
  <div class="grid">${SERIES.map((spec) => chartCard(cardSpecFor(spec), pointsFor(spec, periods))).join('')}</div>
...
<div class="wrap">
  ...
  ${controls}
  ${person ? `<a class="back" href="${person.backHref}">&larr; all engineers</a>` : ''}

  ${person ? person.cards : sections}
</div>
</body>
</html>
`;
